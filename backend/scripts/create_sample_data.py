...
Execute com: python manage.py shell < scripts/create_sample_data.py
"""
import os
import django
...
def create_sample_data():
    ...
    try:
        ...
        print("Usuário admin não encontrado. Crie-o primeiro.")
        return
    ...
    print("Criando vídeos de exemplo...")
    ...
    for video_data in videos_data:
        ...
        videos.append(video)
        print(f"  ✓ Criado: {video.title}")
    
    # Cria rhythm grids para os vídeos
    print("\nCriando rhythm grids...")
    
    rhythm_configs = [
        {'bpm': 120, 'time_sig': (4, 4), 'offset': 0.5},  # Escala Pentatônica
        {'bpm': 90, 'time_sig': (4, 4), 'offset': 1.0},   # Fingerstyle
        {'bpm': 140, 'time_sig': (2, 4), 'offset': 0.3},  # Samba
        {'bpm': 110, 'time_sig': (4, 4), 'offset': 0.8},  # Bossa Nova
    ]
    ...
    for video, config in zip(videos, rhythm_configs):
        ...
        print(f"  ✓ Grid criado para '{video.title}': {config['bpm']} BPM, {config['time_sig'][0]}/{config['time_sig'][1]}")
    
    # Cria fragmentos para alguns vídeos
    print("\nCriando fragmentos...")
    
    # Fragmentos para o primeiro vídeo (Escala Pentatônica)
    fragments_video1 = [
        {
            'name': 'Introdução',
    ...
    for frag_data in fragments_video1:
        ...
        print(f"  ✓ Fragmento criado: {fragment.name} (compassos {fragment.bar_start}-{fragment.bar_end})")
    
    # Fragmentos para o segundo vídeo (Fingerstyle)
    fragments_video2 = [
        {
            'name': 'Aquecimento',
    ...
    print(f"\nResumo:")
    print(f"  - {Video.objects.count()} vídeos")
    print(f"  - {RhythmGrid.objects.count()} rhythm grids")
    print(f"  - {Fragment.objects.count()} fragmentos")
